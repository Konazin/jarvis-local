...
DESKTOP_TOOLS = (
    Tool(
        "get_active_window",
        "Consulta título e classe do X11 active window. Não lê conteúdo interno da janela.",
        _NO_ARGUMENTS,
        RiskLevel.SAFE,
        get_active_window,
    ),
    Tool(
        "get_audio_status",
        "Consulta volume e mute do dispositivo de saída padrão usando PipeWire/wpctl.",
        _NO_ARGUMENTS,
        RiskLevel.SAFE,
        get_audio_status,
    ),
    Tool(
        "set_volume",
        "Define o volume de saída padrão entre 0 e 100 por cento, após confirmação.",
        _PERCENT_ARGUMENTS,
        RiskLevel.CONFIRM,
        set_volume,
    ...
    Tool(
        "toggle_mute",
        "Alterna o mute do dispositivo de saída padrão após confirmação.",
        _NO_ARGUMENTS,
        RiskLevel.CONFIRM,
        toggle_mute,
    ),
    Tool(
        "media_play_pause",
        "Alterna play/pause do player padrão após confirmação.",
        _NO_ARGUMENTS,
        RiskLevel.CONFIRM,
        media_play_pause,
    ),
    Tool(
        "media_next",
        "Avança para a próxima faixa do player padrão após confirmação.",
        _NO_ARGUMENTS,
        RiskLevel.CONFIRM,
        media_next,
    ),
    Tool(
        "media_previous",
        "Volta para a faixa anterior do player padrão após confirmação.",
        _NO_ARGUMENTS,
        RiskLevel.CONFIRM,
        media_previous,
    ),
    Tool(
        "get_network_status",
        "Consulta interfaces de rede ativas e contadores locais; não consulta IP público, credenciais ou rotas.",
        _NO_ARGUMENTS,
        RiskLevel.SAFE,
        get_network_status,
    ),
